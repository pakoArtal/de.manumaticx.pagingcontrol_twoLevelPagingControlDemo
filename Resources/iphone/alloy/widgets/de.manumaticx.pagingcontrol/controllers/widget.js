...
function Controller() {
    ...
    function updateOffset(index) {
        var width = $.pagingcontrol.size.width, tabsWidth = $.tabsCtrl.getWidth(), maxOffset = tabsWidth - width, tabSpace = tabsWidth * index / $.scrollableView.views.length;
        require("alloy/measurement");
        if (tabsWidth > width) {
            var offset = tabSpace - args.scrollOffset, offsetDp = maxOffset > offset ? offset : maxOffset, newOffset = 0 > offsetDp ? 0 : offsetDp;
